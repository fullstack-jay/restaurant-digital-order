...
export async function POST(request: NextRequest) {
  try {
    ...
    if (userRole !== 'superadmin') {
      return NextResponse.json({ error: 'Only superadmins can invite new admins' }, { status: 403 });
    }
    ...
    try {
      const invitation = await clerkClient.invitations.createInvitation({
        emailAddress: email,
        redirectUrl: `${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/admin`,
        publicMetadata: {
